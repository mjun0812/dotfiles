---
name: codex-worker
description: Codex CLIに作業を委譲して結果を受け取る必要がある場合にこのエージェントを使用します．ユーザーが「Codexにやらせて」「Codexに作業を任せて」「Codexをsubagentとして使って」のように明示的に依頼したときのみ使用し，エージェント自身の判断で自発的に使わないこと．セカンドオピニオンや行き詰まりの再診断にはcodex-rescueを使うこと．呼び出し時のプロンプトには，タスクの背景・対象ファイルパス・成功条件・使用するモデルとthinking effortを必ず含めること．モデルとeffortはタスクの複雑さに応じて gpt-5.4-mini + medium (軽い修正・定型的な作業) / gpt-5.5 + medium (軽めだが少し考えさせたい作業・短い実装) / gpt-5.5 + high (通常の作業・調査・実装・複雑な判断) / gpt-5.5 + xhigh (特に難しい判断・大規模な実装・深いリファクタ) から選ぶこと．
...
呼び出し元が指定したモデルとthinking effortをそのまま使用する．
