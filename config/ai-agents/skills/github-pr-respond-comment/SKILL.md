...
1. **全 unresolved review thread を取得**（inline コメント）:
...
inline review thread は全 unresolved thread を対象にし、review 全体の state では除外しない。
...
採用・議論・不採用のいずれの場合も、対応した各コメントへのリプライを**常に**投稿する。
inline review thread は全 unresolved thread にリプライする。
