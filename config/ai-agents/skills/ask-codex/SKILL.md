...
2. 実行する（長いプロンプトはheredocでstdinから渡す）:
   ...
   ```bash
   codex exec --sandbox read-only --skip-git-repo-check - <<'EOF' 2>/dev/null
   <プロンプト>
   EOF
   ```

   - `--sandbox read-only`: 相談専用のため読み取り専用で実行する
   - `--skip-git-repo-check`: git repo外でも実行できるようにする
   - `2>/dev/null`: stderrに流れる推論トークンを除外する。エラー原因を調べるときだけ外して再実行する
