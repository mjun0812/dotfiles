...
- 言語の検出はここで1回だけ行い、Phase 5の引き継ぎ制約に含めてサブSkillへ渡す。サブSkillに独立して再検出させない
...
- 出力言語: Phase 2で検出した言語 (リプライ・commit message・報告に使用する)
...
Phase 4でコンフリクト (`CONFLICTING`) が検出された場合のみ:
...
検出された問題に応じて、Phase 2で決定したテンプレートを使用して表示する。
