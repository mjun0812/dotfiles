...
description: >-
  ...
  受け取った内容をGitHub modeのmjun-specifyへそのまま渡す。
...
受け取った引数と依頼内容に `--github` を付けて、そのまま `mjun-specify` をSkill toolで呼び出してください。Issue作成・調査・decision解決・承認・反映のすべては `mjun-specify` が行います。このskill自身は他に何もしません。
