...
name: mjun-grilling
description: >-
  計画・設計・specに残る意思決定を、ユーザーへの質問で解決するSkill。
  ユーザーが「計画を詰めて」「計画を改善して」「設計を厳しく問い詰めて」「grillして」と依頼したら設計全体を対象にし、
  呼び出し元skillからHuman-owned decisionを1件渡されたらその1件だけを解決する。
...
- 質問の前に、コードベース・既存の規約・ドキュメント・`.mjun/steering/`・用語集 `CONTEXT.md` (repo直下にあればそれ、無ければ `.mjun/CONTEXT.md`)・決定記録 `adr/` (`docs/adr/` があればそれ、無ければ `.mjun/adr/`) の関連箇所を調べる。用語の衝突と既存決定との矛盾は、そのまま質問の論点にする
