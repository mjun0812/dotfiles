...
- `gpt-5.4-mini` + `medium`: 軽い確認・定型的な相談・軽い修正
- `gpt-5.5` + `medium`: 軽めだが少し考えさせたい相談・短い実装
- `gpt-5.5` + `high`: 通常の作業・調査・レビュー・実装・複雑な判断
- `gpt-5.5` + `xhigh`: 特に難しい判断・大規模な調査・実装・深いレビュー。モデルが対応している場合のみ
