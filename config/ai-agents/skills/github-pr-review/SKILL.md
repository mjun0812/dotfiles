...
- 既存レビューを dismiss する場合は `--dismiss-review-id <id>` を追加
...
**重要**: Phase 3 / Phase 4 が例外的に中断した場合も，このクリーンアップは必ず実行する。`trap` のようなシェル機構ではなく，オーケストレーション側の制御フローで「中断 → 直ちに Phase 5 を実行」を保証する。
