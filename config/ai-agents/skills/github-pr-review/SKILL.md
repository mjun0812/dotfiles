...
- reviewer カテゴリと `問題` / `完了条件` (`Problem` / `Completion condition`)は統合して残す
- verifier の「完了条件の評価」を反映し、実装方法を指定せず、問題が解消されたと判断できる状態を `完了条件` に残す
- `証拠` は最終確認用の内部情報として保持し、レビューレポートと inline comment には転記しない
...
レポートの `## 要修正事項` / `## Required Changes` セクション配下の項目のみをinline comment化する。最終レビューにその他の指摘セクションは作成しない。
各項目の直後に続くインデント済みの `問題` / `完了条件` (`Problem` / `Completion condition`)も同じ inline comment 本文に含める。内部情報の `証拠` は含めない。
