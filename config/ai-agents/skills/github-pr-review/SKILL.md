...
Phase 2.2の確定指摘一覧だけを使い、次の順でレビューレポートを作成する。

1. PRのタイトルと本文から出力言語を決める。
   - 主に日本語の場合は `references/report-ja.md`
   - それ以外または判定が曖昧な場合は `references/report-en.md`
2. 指摘の採否、内容、`完了条件`の意味を変えず、`要約`、`問題`、`完了条件`の表現を出力言語に合わせる。`証拠` はレポートに含めない。
3. Verdictを決める。
   - 確定指摘が1件以上の場合は `REQUEST_CHANGES`
   - 確定指摘が0件の場合は `APPROVE`
   - self reviewを含め、レポート内では `COMMENT` を使用しない。GitHub APIへ渡すeventはPhase 3.1で決める
4. 選択したテンプレートを埋めてレポート本文を生成する。
...
確定指摘の `(path, line, side)` が PR の diff に含まれるか検証し、diff内の指摘からinline comments JSONを生成する。
レポート本文を解析してinline commentsを作らず、Phase 2.2の同じ確定指摘一覧と出力言語から生成する。
`証拠` はinline commentに含めず、番号・内容はレポート本文と一致させる。
