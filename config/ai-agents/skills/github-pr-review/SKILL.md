...
5. **self review 判定**:
   ...
   - 一致する場合は `is_self_review = true` を**メインオーケストレーション側で保持する**（reviewer agent には渡さない。Phase 3 の API event 決定でのみ使用）
...
6. **worktree の作成**:
   ...
   - **重要**: 作成した worktree と一時 ref は Phase 4 で必ず削除する
...
各 agent への指示には [reviewer 共通指示テンプレート](#reviewer-共通指示テンプレート) を必ず含めること．
...
1. **5つの agent の結果を統合する**:
   - 各 agent の Must Fix / Should Fix / Questions を集約する
   - **重複統合ルール**:
     - 同じ `filepath:line`（または同じファイル内の隣接行 ±2 行）への指摘は 1 項目に統合する
     - 統合時の重みは Must Fix > Should Fix > Questions（より重い側に寄せる）
     - カテゴリは複数併記する（例: `[code-quality / security]`）
     - 文面は短い方を残しつつ，固有情報は `Evidence` / `Suggested fix`（日本語では `根拠` / `推奨対応`）に統合する
     - `Confidence` / `確信度` は統合前の最も低い値に寄せる
     - `Blocking reason` / `ブロック理由` は Must Fix の場合のみ保持し，複数ある場合は [指摘分類の判定基準](#指摘分類の判定基準) の候補から最も直接的な理由を1つ選ぶ
   - **ノイズ削減ルール**:
     ...
     - `Confidence: Low` の指摘は，レビュー判断に必要な確認事項として明確に書ける場合のみ Questions に回す。実害が説明できない場合や任意提案に過ぎない場合は除外する

2. **Must Fix / Should Fix / Questions の判定基準**:
   - [指摘分類の判定基準](#指摘分類の判定基準) に従う
...
4. **レビューレポートを生成する**:
   ...
   - **重要**: "Must Fix" / "Should Fix" の項目はセクション内で **`1.` から始まる番号付きリスト** で出力すること。各項目の先頭行は ``N. `filepath:line` - [category] description`` の形式を厳密に守る（[Inline comments 抽出パターン](#ステップ-1-レポートから-inline-comments-を抽出する) と一致させる）。番号は Must Fix と Should Fix のそれぞれで 1 から振り直す
   - **重要**: 補助項目（Impact / Evidence / Suggested fix / Confidence / Blocking reason、日本語は影響 / 根拠 / 推奨対応 / 確信度 / ブロック理由）は親項目に対して **3 スペースインデント** の `-` でぶら下げる（番号付きリストの継続インデントは番号桁数 + ピリオド + スペース = 3 文字）。Must Fix には `Blocking reason` / `ブロック理由` も記述すること
   - **重要**: Must Fix の番号は inline comment 本文の prefix (`🔴 **Must Fix #N**:` / `🔴 **要修正 #N**:`) と必ず一致させること
...
6. **GitHub への投稿を確認する**:
   ...
   - ユーザーが投稿を選択した場合は [scripts/post_review.sh](scripts/post_review.sh) を呼び出す。詳細は [Inline comments 付きレビューの投稿](#inline-comments-付きレビューの投稿) を参照。
     - **重要**: レビュー本文・inline comment本文は必ずファイルに保存してから投稿する。`gh pr review --body "..."`、`gh api -f body="..."`、シェル上で組み立てたJSON文字列の直接渡しは禁止
...
| -------------- | ------------------- | ------------ |
| ブロック指摘   | Must Fix            | 要修正       |
| 非ブロック指摘 | Should Fix          | 修正推奨     |
| 確認事項       | Questions           | 質問         |
| 影響           | Impact              | 影響         |
| 根拠           | Evidence            | 根拠         |
| 推奨対応       | Suggested fix       | 推奨対応     |
| 確信度         | Confidence          | 確信度       |
| ブロック理由   | Blocking reason     | ブロック理由 |
| 確信度の値     | High / Medium / Low | 高 / 中 / 低 |

`Blocking reason` / `ブロック理由` は以下のいずれかにする:
...
| data loss        | データ破壊          |
| regression       | 回帰                |
| public API break | 公開APIの破壊的変更 |
| requirement gap  | 要件未達            |

### 指摘分類の判定基準

- **Must Fix / 要修正**: `Confidence: High` / `確信度: 高` で，`Evidence` / `根拠` と `Blocking reason` / `ブロック理由` が明確なものだけ。対象はバグ，セキュリティ脆弱性，データ破壊，明確な regression，未告知の公開 API 破壊，PR の目的未達成に限定する
- **Should Fix / 修正推奨**: Must Fix ではないが修正価値が説明できるもの。`Impact` / `影響`，`Evidence` / `根拠`，`Suggested fix` / `推奨対応`，`Confidence` / `確信度` を必ず書く。確信度は High / Medium（日本語では高 / 中）のみ
- **Questions / 質問**: レビュー判断に必要な確認事項のみ。任意提案，好みの改善案，実害が説明できない低確信度の指摘は含めない
...
>
> **`filepath:line` の行番号制約（重要）**: `filepath:line` の `line` は，必ず PR の diff に含まれる行（新規追加・変更された行，差分中の context 行，削除行）でなければなりません．worktree 全体は文脈把握のために読むものであって，行指定そのものに使ってはいけません．削除された行に対する指摘は LEFT 側の行番号を使い，`` `filepath:line (side=LEFT)` `` のように side を明示してください（明示がない場合は変更後ファイルの行番号 = RIGHT として扱われます）．diff に含まれない行に指摘したい場合は，最も近い変更行を指して本文中で位置を説明するか，Questions セクションに回してください．
>
> **出力形式**: Must Fix / Should Fix / Questions の3セクションで結果を出力してください．Must Fix / Should Fix の各項目の先頭行は `` `filepath:line` - [category] description `` の形式にし，直下に Impact / Evidence / Suggested fix / Confidence を書いてください．Must Fix には Blocking reason も必要です．日本語で出力する場合は，要修正 / 修正推奨 / 質問，影響 / 根拠 / 推奨対応 / 確信度 / ブロック理由，高 / 中 / 低を使ってください．
>
> **番号付与について**: 最終レポートでは Must Fix / Should Fix が `1.` から始まる番号付きリストになりますが，番号は Phase 3 の統合フェーズで他 agent の結果とマージした後にオーケストレーション側が振ります．reviewer agent 側の出力は `- ` 始まりの bullet で構いません．
>
> **判定基準**: Must Fix は High confidence かつ明確な Evidence と Blocking reason がある場合のみ。Should Fix は High / Medium confidence で修正価値を説明できる場合のみ。Questions はレビュー判断に必要な確認事項だけに限定し，任意提案は含めないでください．
>
...
Must Fix / 要修正 の項目はレポート上で `1.` から始まる番号付きリストになっている。抽出時にこの **項目番号（`<index>`）も捕捉**し，inline comment 本文の prefix に `#<index>` として埋め込む（レポート本文と inline 側を相互参照できるようにするため）。

抽出パターン（正規表現の例）: ``^(?<index>\d+)\.\s+`(?<path>[^:]+):(?<line>\d+)` - \[(?<category>[^\]]+)\] (?<comment>.+)$``

各 Must Fix / 要修正 項目の直後に続くインデント済みの補助項目（Impact / Evidence / Suggested fix / Confidence / Blocking reason）も，同じ inline comment の本文に含める。次のトップレベル番号付き項目または次のセクション見出しに到達したら、その Must Fix 項目の本文は終了とみなす。

各項目を以下のJSON形式に変換し，配列としてファイルに保存する（例: `/tmp/pr-review-comments.json`）。inline comment の本文もJSONファイル内の `body` として保存し、コマンド引数へ直接埋め込まない:
...
`#1` はレポート本文の Must Fix 番号と一致させること。日本語レポートの場合は `🔴 **要修正 #1**:` のように prefix を切り替える。
...
投稿時はこのファイルを `--body-file` として渡す。本文を `--body` や `-f body=...` で直接渡すことは禁止。
...
このスクリプトは `jq --rawfile` で本文ファイルを読み込み、`gh api --input <payload-file>` で投稿する。レビュー本文やinline comment本文をシェル引数として直接渡さないこと。
...
| ---------- | ------------------- | ----------------- |
| Must Fix   | 🔴 **Must Fix #N**: | 🔴 **要修正 #N**: |

`#N` はレポート本文 "Must Fix" / "要修正" セクション内での項目番号（1 始まり）に置換すること。レポート本文と inline comment の番号は必ず一致させる。

Should Fix は inline 化しないため prefix は不要（本文記載のみ。ただし本文では `1.` から始まる番号付きリストとして記載する）。

### 注意事項

- 行番号は，`side` が `RIGHT`（既定）の場合は変更後ファイル（diff の右側）の行に，`LEFT` の場合は変更前ファイル（diff の左側）の行に対応していなければならない
- `post_review.sh` は投稿前に PR の files API から各ファイルの patch を取得し，`(path, line, side)` が diff に含まれているかをプリフライト検証する。invalid なエントリは自動的にレビュー本文末尾の「inline 化できなかった指摘 / Comments that could not be inlined」セクションへ降格され，残りの inline 投稿は継続される（422 エラーで全件失敗するのを防ぐため）。降格件数は標準エラーに `Warning:` として出力される
- inline 化できる可能性を最大化するため，reviewer agent は最初から diff に含まれる行を選ぶこと（[Reviewer 共通指示テンプレート](#reviewer-共通指示テンプレート) の行番号制約を参照）
