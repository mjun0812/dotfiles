...
Reviewは2段構成で行う。「mergeを止めるリスク」の種類ごとに分かれた finder SubAgent を並列実行して Must Fix 候補を収集し（Phase 3.2）、候補1件ごとに verifier SubAgent が反証を試みて false positive を落とす（Phase 3.3）。
...
SubAgent ツールを使い、PR の変更内容に関係する finder SubAgent を選択して並列起動する。起動対象は Phase 3.1 で収集した情報から判断する。
判断に迷う場合は関連しそうな SubAgent を広めに起動する。大規模変更・横断変更・影響範囲が不明な変更では全 finder を起動してよい。
起動時は model に sonnet を指定する（model 指定ができない環境ではそのまま起動する）。
SubAgent機能が使えない環境では、各観点のレビューをメイン会話内で順に実施する。

起動可能な finder は以下の5つで、それぞれが「mergeを止めるリスク」への問いを1つ持つ。
...
**候補選別**: 各 finder の出力から Must Fix 候補を確定する。

- finder の `[must]` はそのまま候補とする
- `[should]` / `[question]` は、「このまま merge してはいけない」と判断できる根拠が揃っている場合のみ候補へ昇格させ、それ以外は破棄する
- 候補は以下をすべて満たすこと: PR の diff が導入・露出した問題である / `証拠` の実行パスがある / 影響が具体的である / `確度` が `high` または `medium` である
...
- 起動時は model に opus を指定する（反証は難度の高い調査判断のため。model 指定ができない環境ではそのまま起動する）
...
最終レビューの `Must Fix` に載せてよいのは、Phase 3.3 で verifier が `confirmed` と判定した指摘のみ。それ以外（finder の `[should]` / `[question]`、verifier が `refuted` / `uncertain` とした候補）は最終レビューに出さない。

統合時は以下を行う。

- 同じ `filepath:line` または同じ根本原因の指摘が残っていれば 1 件にまとめる
- reviewer カテゴリと `理由` / `影響` / `対応` / `確度` / `証拠`（`Reason` / `Impact` / `Action` / `Confidence` / `Evidence`）は統合して残す
