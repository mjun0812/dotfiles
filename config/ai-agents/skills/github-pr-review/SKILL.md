...
4. **レビューレポートを生成する**:
   ...
   - **重要**: Must Fix / Should Fix の補助項目と確信度は [出力ラベル対応](#出力ラベル対応) に従う。Must Fix には `Blocking reason` / `ブロック理由` も記述すること
...
>
> **出力形式**: Must Fix / Should Fix / Questions の3セクションで結果を出力してください．Must Fix / Should Fix の先頭行は `` `filepath:line` - [category] description `` の形式にし，直下に Impact / Evidence / Suggested fix / Confidence を書いてください．Must Fix には Blocking reason も必要です．日本語で出力する場合は，要修正 / 修正推奨 / 質問，影響 / 根拠 / 推奨対応 / 確信度 / ブロック理由，高 / 中 / 低を使ってください．
>
...
各 Must Fix / 要修正 項目の直後に続くインデント済みの補助項目も，同じ inline comment の本文に含める。次のトップレベル項目または次のセクション見出しに到達したら、その Must Fix 項目の本文は終了とみなす。
