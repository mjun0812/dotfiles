...
- `--dry-run`: 指定時は `github-pr-create` へ `--dry-run` を転送し、PR内容のプレビュー提示で停止する。Phase 2以降（PR取得・レビュー）は実行しない
...
2. 受け取った引数（`language` / `--draft` / `--reviewer` / `--label` / `--dry-run`）をそのまま転送する
3. `--dry-run` 指定時は `github-pr-create` のプレビュー提示で停止し、Phase 2以降は実行しない
4. 完了状態を確認し、以下のいずれかなら Phase 2 に進む:
   - 新規PR作成に成功
   - 既存PRが更新された
   - 既存PRに対してpushのみ実行された
5. エラー・中止の場合は本Skillも中止し、理由を伝える（レビューは行わない）
...
- レビュー投稿前の確認は委譲先（`github-pr-review`）の確認フローに従う
