...
4. 投稿確認のプロンプトはユーザーに必ず提示する。**セルフレビューであっても自動投稿しない**
...
- **Review verdict**: 投稿したセルフレビューの判定（`APPROVE` / `REQUEST_CHANGES` / `COMMENT`）。投稿をスキップした場合は `not posted`
- **指摘件数**: Must Fix / Should Fix / Good Points の件数
