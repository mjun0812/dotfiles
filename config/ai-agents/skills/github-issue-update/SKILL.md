...
description: open issueを横断的に点検して、closeすべきもの・内容を追記すべきもの・ラベルを追加/削除すべきものを自動判定し、最後にsummaryを提示してユーザー承認の上で一括反映するSkill。「open issueを整理して」「stale issueを片付けて」「ラベルを整理して」のような依頼で使う。
...
各open issueに対して以下を判定する。**迷ったら候補から外す**（ノイズより安全側を優先。最終承認はユーザーに委ねるため、Phase 1段階で完璧を目指す必要はない）。
