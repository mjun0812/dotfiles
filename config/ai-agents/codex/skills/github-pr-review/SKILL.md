...
description: GitHub pull requestのコードレビューを実行し、統合レポートを作成する。
---
...
並列実行を使い，以下の5つのagentを**すべて同時に並列起動**すること．
...
```yaml
review_focus: code-quality-reviewer
```
...
```yaml
review_focus: documentation-accuracy-reviewer
```
...
```yaml
review_focus: performance-reviewer
```
...
```yaml
review_focus: security-reviewer
```
...
5. **GitHub への投稿を確認する**:
   - ユーザー確認 ツールを使い，レビューを GitHub に投稿するかユーザーに確認する
   - ユーザーが投稿を選択した場合:
...
```markdown
# <img src="https://github.com/openai.png?size=32" alt="Claude Icon" width="32" height="32" style="vertical-align:middle; margin-right:8px;" /> Codex Review Pull Request
