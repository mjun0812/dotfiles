...
-- https://github.com/Ha1baraA11/Chrome-Vertical-Tab-Sidebar-Toggle/blob/main/init.lua
-- Hammerspoon script to toggle Chrome's native vertical tab sidebar
-- via keyboard shortcut (Cmd+B) and/or mouse left-edge hover.
...
--   1 = Keyboard only      — Cmd+B toggles sidebar
...
-- Button label matching (locale-tolerant).
-- Chrome localizes the collapse/expand button's AXTitle / AXDescription to the
-- browser UI language, so an exact English match ("expand tabs"/"collapse tabs")
-- fails on non-English Chrome. Instead we match when the label contains BOTH a
-- "tab" word AND a toggle word, which stays robust across locales.
--
-- Observed on a Japanese Chrome (June 2026): the button lives in the vertical
-- AXTabGroup, is an AXButton, and carries its label in AXDescription as
-- "タブを開く" when collapsed (toggling to "タブを閉じる" when expanded) — note
-- the verb is 開く/閉じる, not 展開/折りたたむ. If your locale differs, run
-- Cmd+Alt+B to inspect and add the wording here.
local TAB_WORDS           = { "tab", "タブ" }
local TOGGLE_WORDS        = { "expand", "collapse", "展開", "折りたた", "折り畳", "開く", "閉じる" }
...
-- ----------------------------------------------------------
-- AX: Locale-tolerant label matching
-- ----------------------------------------------------------
local function containsAny(haystack, words)
	for _, w in ipairs(words) do
		if haystack:find(w, 1, true) then return true end
	end
	return false
end

-- True when a label looks like Chrome's tab-strip collapse/expand button,
-- regardless of UI language (matches if it has both a "tab" and a toggle word).
isToggleLabel             = function(s)
	s = string.lower(tostring(s or ""))
	s = s:gsub("^%s+", ""):gsub("%s+$", "")
	if s == "" then return false end
	return containsAny(s, TAB_WORDS) and containsAny(s, TOGGLE_WORDS)
end
...
findSidebarButton         = function(axElement, depth)
	depth = depth or 0
	if not axElement or depth > 20 then return nil end

	local role = axElement:attributeValue("AXRole")

	-- The toggle is part of Chrome's native UI (inside the vertical AXTabGroup),
	-- never inside page content. Skip AXWebArea subtrees so a web page's own
	-- "collapse/expand tab" button can't be matched by mistake, and to avoid
	-- walking the entire DOM.
	if role == "AXWebArea" then return nil end

	if role == "AXButton" then
		if isToggleLabel(axElement:attributeValue("AXTitle"))
			or isToggleLabel(axElement:attributeValue("AXDescription"))
			or isToggleLabel(axElement:attributeValue("AXHelp")) then
			return axElement
		end
	end
...
createKeyTap              = function()
	...
	keyTap = eventtap.new({ eventtap.event.types.keyDown }, function(event)
		...
		if flags.cmd and not flags.ctrl and not flags.alt and not flags.shift
			...
			log("Cmd+B intercepted, toggling sidebar")
			toggleSidebar()
			return true
		end
...
hs.hotkey.bind({ "cmd", "alt" }, "B", function()
	...
	local function dumpButtons(el, depth)
		if not el or depth > 20 then return end
		...
		local ident = el:attributeValue("AXIdentifier")

		-- Dump every button, plus any element whose text mentions a tab,
		-- so the collapse/expand control shows up even if it is not an AXButton
		-- or its label is localized.
		local blob = string.lower(table.concat({
			tostring(title or ""), tostring(desc or ""), tostring(help or ""),
		}, " "))
		local tabRelated = blob:find("tab", 1, true) or blob:find("タブ", 1, true)

		if role == "AXButton" or tabRelated then
			table.insert(results, string.format(
				"Role: %s | Title: [%s] | Desc: [%s] | Help: [%s] | Id: [%s]",
				role, tostring(title), tostring(desc), tostring(help), tostring(ident)
			))
		end
