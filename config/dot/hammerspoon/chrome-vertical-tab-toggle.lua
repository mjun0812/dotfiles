...
local function findSidebarButton(axElement, depth)
  depth = depth or 0
  if not axElement or depth > AX.maxDepth then
    return nil
  end
...
local function getFocusedChromeWindow()
  if not isFrontChrome() then
    return nil
  end
  local axApp = hs.axuielement.applicationElement(app.frontmostApplication())
  return axApp:attributeValue("AXFocusedWindow") or (axApp:attributeValue("AXWindows") or {})[1]
end
...
local function getSidebarButton(win)
  ...
  if (now - runtime.lastMissLog) >= MISS_LOG_INTERVAL then
    ...
    log.w(
      "Sidebar button (Expand tabs / Collapse tabs) not found in Chrome's AX tree. "
...
local function collapseSidebarIfOwned()
  ...
  if not runtime.openedByEdge then
    return
  end
...
local function mousePollCallback()
  ...
  elseif runtime.isEdgeActive and runtime.sidebarRightX and pos.x >= runtime.sidebarRightX + EDGE.exitMarginPx then
...
local function createWatchdog()
  ...
  runtime.watchdogTimer = timer.new(WATCHDOG.intervalSeconds, function()
    ...
    -- race with the first.
    if runtime.isRestarting or runtime.inGrace or not isFrontChrome() then
      return
    end
...
local function setGracePeriod(seconds)
  ...
  if runtime.graceTimer then
    runtime.graceTimer:stop()
  end
...
local function createKeyTap()
  ...
  runtime.keyTap = eventtap.new({ eventtap.event.types.keyDown }, function(event)
    ...
    if event:getKeyCode() ~= TOGGLE_KEYCODE then
      return false
    end

    local flags = event:getFlags()
    if (flags.cmd or false) ~= TOGGLE_MODS.cmd then
      return false
    end
    if (flags.ctrl or false) ~= TOGGLE_MODS.ctrl then
      return false
    end
    if (flags.alt or false) ~= TOGGLE_MODS.alt then
      return false
    end
    if (flags.shift or false) ~= TOGGLE_MODS.shift then
      return false
    end
...
local function startServices()
  if FEATURES.keyboardToggle and not (runtime.keyTap and runtime.keyTap:isEnabled()) then
    createKeyTap()
    if runtime.keyTap then
      runtime.keyTap:start()
    end
  end
  ...
  if FEATURES.mouseEdgeToggle then
    if not (runtime.mousePoller and runtime.mousePoller:running()) then
      createMousePoller()
      if runtime.mousePoller then
        runtime.mousePoller:start()
      end
    end
    if not (runtime.watchdogTimer and runtime.watchdogTimer:running()) then
      createWatchdog()
      if runtime.watchdogTimer then
        runtime.watchdogTimer:start()
      end
    end
  end
end
...
local function stopServices()
  if runtime.keyTap and runtime.keyTap:isEnabled() then
    runtime.keyTap:stop()
  end
  if runtime.mousePoller and runtime.mousePoller:running() then
    runtime.mousePoller:stop()
  end
  if runtime.watchdogTimer and runtime.watchdogTimer:running() then
    runtime.watchdogTimer:stop()
  end
  resetEdgeState()
end
...
appWatcher
  .new(function(appName, eventType, _)
...
log.i(
  string.format(
    "Features enabled: keyboardToggle=%s mouseEdgeToggle=%s",
    tostring(FEATURES.keyboardToggle),
    tostring(FEATURES.mouseEdgeToggle)
  )
)

timer.doAfter(DELAY.init, function()
  if isFrontChrome() then
    startServices()
  end
