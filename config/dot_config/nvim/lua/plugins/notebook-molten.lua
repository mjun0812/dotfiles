...
local function current_percent_cell()
  ...
  while first_line <= last_line and lines[first_line] and lines[first_line]:match("^%s*$") do
    first_line = first_line + 1
  end

  while last_line >= first_line and lines[last_line] and lines[last_line]:match("^%s*$") do
...
return {
  {
    ...
    config = function(_, opts)
      ...
      vim.api.nvim_create_autocmd("BufReadPost", {
        group = vim.api.nvim_create_augroup("jupytext-filetype-refresh", { clear = true }),
        pattern = "*.ipynb",
