...
(defun elmo-imap4-send-command-build-queue (tag tokens literal+)
  (with-temp-buffer
    (insert tag " ")
    (let ((queue (elmo-imap4-send-command-enqueue tokens literal+)))
      (unless (bobp)
        (delete-char -1)
        (push (buffer-string) queue))
      (nreverse queue))))
...
SESSION is a network session.
COMMAND is a list of command tokens or a single command token."
  (let* ((tag (elmo-imap4-command-tag session))
         (queue (elmo-imap4-send-command-build-queue
                 tag
                         ...
                         (mapcar
                          (lambda (namespace)
                            ...
                            (if (zerop (length prefix))
                              ...
                              (cons
                               (concat "^\\("
                                       (if (string= (downcase prefix) "configured note path")
                                           "[Ii][Nn][Bb][Oo][Xx]"
                                         (regexp-quote prefix))
                                       "\\).*$")
                               delim)))
...
(luna-define-method elmo-folder-expand-msgdb-path ((folder
  ...
  (convert-standard-filename
   (let ((mailbox (elmo-imap4-folder-mailbox-internal folder)))
     (if (string= "configured note path" (downcase mailbox))
         (setq mailbox "configured note path"))
