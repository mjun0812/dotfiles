...
| [`mjun-design-review`](../config/ai-agents/skills/mjun-design-review/SKILL.md) | 実装設計 (`.mjun/specs/<slug>` のdesign.md、任意の設計Markdown、会話中の設計) を敵対的にレビューする。fresh contextのreviewerがcontractの充足・Boundaries・コードベースとの整合・失敗経路・Test Seam・構造の過不足を検査して候補を出し、候補ごとにverifierが実コードとcontractの明文で反証してconfirmedだけを深刻度順にチャットへ返す。ファイルは編集しない                                                                                                                                                                                                           |
...
| `mjun-specify`   | `mjun-design-review`                                             | Phase 5.5 (承認前) でdesign.mdを検査。contract reviewと同じ規則で指摘を反映    |
