...
| [`mjun-implement`](../config/ai-agents/skills/mjun-implement/SKILL.md) | specのcontract承認状態と内容を検査し (Issue番号はspecify済みspecへの逆引きで解決)、worktreeでtaskごとにimplementer/reviewer SubAgentを回して実装、Acceptance Criteria照合を経てcommitと必要ならPRまで一気通貫。task進捗と選択branchを永続化し中断後にresumeできる   |
