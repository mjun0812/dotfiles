...
| [`mjun-implement`](../config/ai-agents/skills/mjun-implement/SKILL.md)           | specを内容検査し (Issue番号は逆引き・自動取込みで解決)、worktreeでtaskごとにimplementer/reviewer SubAgentを回して実装、Acceptance Criteria照合を経てcommitと必要ならPRまで一気通貫。task進捗を永続化し中断後にresumeできる                                          |
