...
`mjun-` prefixの自作開発フローskill群。specの正本は常に `.mjun/specs/` のLocal specで、GitHub Issueは取り込みと投影のアダプタとして扱う。入口は `mjun-specify` で、contract承認後に必要ならtask分解し、`mjun-implement` で実装からPRまで進める。`.mjun/CONTEXT.md` (用語集) と `.mjun/adr/` (決定記録) は各skillが作業前に読み、用語は `mjun-specify` が確定時に、ADRは `mjun-implement` が配送完了時に `decisions.md` から3条件 (覆しにくい / 文脈なしでは不可解 / 本物のtrade-off) を満たす決定だけを書く。
...
| [`mjun-steering`](../config/ai-agents/skills/mjun-steering/SKILL.md)       | `.mjun/steering/` を実装にgroundされたproject memoryとして生成 (Bootstrap)・drift検出付きで追記更新 (Sync) し、履歴 (merged PR / closed Issue / docs) から用語集 `.mjun/CONTEXT.md` と由来付きADR `.mjun/adr/` を発掘して追記する (追記専用)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
