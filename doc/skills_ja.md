...
| [`mjun-implement`](../config/ai-agents/skills/mjun-implement/SKILL.md)     | specのcontract承認状態と内容を検査し (Issue番号はspecify済みspecへの逆引きで解決、依存specの完了を確認)、tasks.mdに記録したbranchからresumeしてtaskごとに実装する。reviewerはdiffのパスをdesign.mdのChange Outlineと機械照合し、配送前にbase branchへrebaseして全検査を再実行する。specとtask双方のAcceptance Criteriaとcontract境界の照合を経てcommitと必要ならPRまで進める。taskごとにverifier SubAgentがAcceptance Criteriaを失敗する検査に落としてから実装し (検査はハッシュで改変検出)、検証不能時は別方式を有界に試す。reviewerの承認後は親が検査とハッシュ照合を再実行してからcommitし、SubAgentの報告を検証の代わりにしない。大きすぎるtaskやtask分解・順序の問題はcontract不変を検査して自動修正し、解決できないtaskを隔離して独立taskを続行する。残ったblocked taskと依存待ちは最後に原因・再開条件付きで報告する。収束しない実装はfresh contextのdebuggerが分類し、feature検証 (smoke、AC matrix、task間整合) とrefactor passを経て配送する。各SubAgentはleaf roleとして再移譲せず、受け渡しと追加の委譲はメイン会話だけが担う |
