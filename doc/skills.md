...
| [`mjun-implement`](../config/ai-agents/skills/mjun-implement/SKILL.md)           | Gate the spec on content (issue numbers resolve via Source lookup or auto-import), implement task by task in a worktree via implementer/reviewer subagents, check acceptance-criteria coverage, then commit and optionally open a PR; task progress persists for resume                                                                        |
